...
const CATEGORIES_FALLBACK = [
  "Breaking News", "Sports", "Arts & Culture",
...
export default function CreateStory() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const story = await createStory(form, user.$id);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <p className="text-gray-500 text-sm mb-8">Share the latest news with your school community.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {form.image_url ? (
            ...
            <label className="flex flex-col items-center justify-center aspect-video rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 cursor-pointer hover:border-black transition-colors">
              {uploading ? <span className="text-sm text-gray-400">Uploading...</span> : (
                <><span className="text-3xl mb-2">🖼️</span><span className="text-sm text-gray-400">Click to upload a cover image</span></>
              )}
        ...
        <div>
          ...
          <input required type="text" placeholder="Enter your headline..." value={form.title}
        ...
        <div>
          ...
          <textarea placeholder="A brief description..." value={form.summary}
            onChange={(e) => update("summary", e.target.value)} rows={2}
        ...
        <div>
          ...
          <textarea required placeholder="Write your story here..." value={form.content}
            onChange={(e) => update("content", e.target.value)} rows={14}
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={submitting || uploading || !form.title || !form.category}
